...
public class OneWithTheForestOne extends Skill {

    private static SkillsManager skillsManager;

    public OneWithTheForestOne(int id, int cost) {
        super(id, "One With the Forest 1", "Your bone meal affects multiple saplings, you find the saplings of other trees in leaves, and bees are more likely to appear on the trees you grow.", cost);

        ItemStack icon = new ItemStack(Material.POPPY);
        ItemMeta meta = icon.getItemMeta();
        meta.displayName(ColorParser.of("<green><bold>" + super.getName() + "</green>").build());

        List<Component> loreList = new ArrayList<>();
        loreList.add(ColorParser.of("<yellow>Level 6</yellow>").build());
